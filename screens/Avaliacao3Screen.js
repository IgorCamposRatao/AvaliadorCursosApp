// screens/Avaliacao3Screen.js
import React, { useState } from 'react';
import { View, Text, StyleSheet, TouchableOpacity } from 'react-native';
import { Ionicons } from '@expo/vector-icons';

export default function Avaliacao3Screen({ navigation, route }) {
  const { curso, avaliacoes } = route.params;
  const [nota, setNota] = useState(0);

  const handleAvancar = () => {
    navigation.navigate('Avaliacao4', {
      curso,
      avaliacoes: {
        ...avaliacoes,
        didatica: nota,
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.tema}>3. Comunicação e Didática</Text>

      <View style={styles.bullets}>
        <Text>• A linguagem utilizada foi acessível e adequada ao grupo.</Text>
        <Text>• Houve equilíbrio entre teoria e prática.</Text>
        <Text>• As dinâmicas e exemplos ajudaram na compreensão do tema.</Text>
      </View>
